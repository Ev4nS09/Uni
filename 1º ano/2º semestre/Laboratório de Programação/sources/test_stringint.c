...
int strint_count_while(const char **a, int n, const char *b)
{
	int result = 0;
	int i = 0;
	while(i < n)
...
int until_space(const char *a, char *c)
{
    int result = 0;
    for (int i = 0; a[i]; i++)
    {
...
int strings_tally2(const char **a, int n, int i)
{
	int j = 0;
    int c = 1;
    int u = 0;
	...
	while(j < i && u != -1)
	{
	    ...
	    else if(strcmp(a[i], a[j]) != 0 && j == i - 1)
		{
			c = strint_count_while(a, n, a[i]);	
		    u = -1;
	    }
...
int strings_tally(const char **a, int n, StringInt *b)
{
    ...
    while(i < n)
    {
    	int c = strings_tally2(a, n, i);
    	if(c == 0)
    		i++;
    	else
    	{
    	 b[result++] = string_int(a[i], c);
    	 i++;
    	}
    }
	return result;
